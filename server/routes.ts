...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get("/api/customers/top-spent", requireAuthentication, async (req: AuthenticatedRequest, res) => {
    try {
      ...
      const topCustomers = await storage.getTopSpendingCustomers(limit, since);
  ...
  app.get("/api/customers/top-orders", requireAuthentication, async (req: AuthenticatedRequest, res) => {
    try {
      const validation = customersQuerySchema.safeParse(req.query);
      if (!validation.success) {
        return res.status(400).json({ message: "Invalid query parameters", errors: validation.error.errors });
      ...
      const { limit, since } = validation.data;
      const topCustomers = await storage.getTopFrequentCustomers(limit, since);
      res.json(topCustomers);
    } catch (error) {
      console.error("Error getting top frequent customers:", error);
      res.status(500).json({ message: "Server error" });
    }
  });

  app.get("/api/customers/inactive", requireAuthentication, async (req: AuthenticatedRequest, res) => {
    try {
      const validation = inactiveCustomersQuerySchema.safeParse(req.query);
      if (!validation.success) {
        return res.status(400).json({ message: "Invalid query parameters", errors: validation.error.errors });
      }
      const { days } = validation.data;
      const inactiveCustomers = await storage.getInactiveCustomers(days);
  ...
  app.get("/api/customers/:id/stats", requireAuthentication, async (req: AuthenticatedRequest, res) => {
    try {
      const { id } = req.params;
  ...
  app.get("/api/customers/overview", requireAuthentication, async (req: AuthenticatedRequest, res) => {
    try {
      const overview = await storage.getCustomersOverview();
      res.json(overview);
    } catch (error) {
      console.error("Error getting customers overview:", error);
      res.status(500).json({ message: "Server error" });
    }
  });

  // Message sending for inactive customers
  app.post("/api/messages/send-inactive", requireAuthentication, async (req: AuthenticatedRequest, res) => {
    try {
      const validation = sendInactiveMessageSchema.safeParse(req.body);
      if (!validation.success) {
        return res.status(400).json({ message: "Invalid request data", errors: validation.error.errors });
      }
      const { days: daysThreshold, templateId } = validation.data;
      
      // Get inactive customers
      const inactiveCustomers = await storage.getInactiveCustomers(daysThreshold);
      
      // Get WhatsApp config
      const whatsappConfig = await storage.getWhatsappConfig();
      if (!whatsappConfig || !whatsappConfig.enabled) {
        return res.status(400).json({ message: "WhatsApp not configured or disabled" });
      }
      
      // Get message template or use default
      let messageTemplate = "¡Hola {customerName}! Te extrañamos en nuestra lavandería. ¿Necesitas algún servicio? ¡Estamos aquí para ayudarte!";
      if (templateId) {
      ...
      for (const customer of inactiveCustomers) {
        ...
        // Skip customers without valid phone numbers
        if (!customer.phone || customer.phone.trim() === '') {
          results.push({ customerId: customer.id, customerName: customer.name, status: 'no_phone' });
          continue;
        }
        
        try {
          const personalizedMessage = messageTemplate.replace('{customerName}', customer.name);
          const success = await sendWhatsAppMessage(customer.phone, personalizedMessage, whatsappConfig);
          if (success) {
            sent++;
            results.push({ customerId: customer.id, customerName: customer.name, status: 'sent' });
          } else {
            results.push({ customerId: customer.id, customerName: customer.name, status: 'failed' });
          }
        } catch (error) {
          console.error(`Error sending message to ${customer.name}:`, error);
          results.push({ customerId: customer.id, customerName: customer.name, status: 'error' });
        }
      }
      ...
      console.error("Error sending inactive customer messages:", error);
      res.status(500).json({ message: "Server error" });
    }
  });
